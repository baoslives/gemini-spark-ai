...
import { X, Instagram, Facebook, Upload, Sparkles, Heart, MessageCircle, Send, Bookmark, MoreHorizontal, Calendar as CalendarIcon, Clock } from "lucide-react";
              ...
              <div className="grid grid-cols-3 gap-2">
                {media.map((img, index) => (
                  <div key={index} className="aspect-square rounded-lg overflow-hidden">
                    <img src={img} alt={`Media ${index + 1}`} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
